...
use NexaMerchant\Shopify\Models\ShopifyStore;
...
class GetV4 extends Command
{
    ...
    public function handle()
    {
        ...
        $shopifyProduct = \NexaMerchant\Shopify\Models\ShopifyProduct::where("product_id", $shopify_pro_id)->first();
        if(!is_null($shopifyProduct) && $force) {
            if($force==true) {
                \NexaMerchant\Shopify\Models\ShopifyProduct::where("product_id", $shopify_pro_id)->delete();
            }else{
        ...
        foreach($body['products'] as $key=>$item) {
            $shopifyProduct = \NexaMerchant\Shopify\Models\ShopifyProduct::where("product_id", $item['id'])->first();
            if(is_null($shopifyProduct)) {
                $shopifyProduct = new \NexaMerchant\Shopify\Models\ShopifyProduct();
    ...
    public function syncProductToLocal($shopify_pro_id) {
        $items = \NexaMerchant\Shopify\Models\ShopifyProduct::where("shopify_store_id", $this->shopify_store_id)->where("product_id", $shopify_pro_id)->get();
        foreach($items as $key=>$item) {
            ...
            $error = 0;
            $LocalOptions = [];
            $LocalOptions = \NexaMerchant\Shopify\Helpers\Utils::createOptions($options);
            ...
            \NexaMerchant\Shopify\Helpers\Utils::clearCache($id, $item['product_id']); // clear cache
           ...
           // $updateData['compare_at_price'] = $item['compare_at_price'];
            ...
            \NexaMerchant\Shopify\Helpers\Utils::send($item['product_id']. " sync done, please check it ");
