...
namespace NexaMerchant\Shopify\Console\Commands\Order;
        ...
        if ($force) {
             ...
             */
             ...
             foreach($items as $key=>$item) {
                $shopOrder = \NexaMerchant\Shopify\Models\ShopifyOrder::where("shopify_store_id", $this->shopify_store_id)->select(['shopify_order_id'])->where("order_id", $item->id)->first();
            ...
            foreach($body['orders'] as $key=>$item) {
                ...
                if(is_null($shopifyNewOrder)) {
                    $shopifyNewOrder = new \NexaMerchant\Shopify\Models\ShopifyOrder();
