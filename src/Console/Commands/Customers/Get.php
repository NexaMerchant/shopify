...
namespace NexaMerchant\Shopify\Console\Commands\Customers;
        ...
        foreach($body['customers'] as $customer) {
            ...
            if($customer['email_marketing_consent']['state'] == "not_subscribed") { //
                ...
                try {
                    ...
                    \NexaMerchant\Shopify\Helpers\Utils::send($e->getMessage().'--' .$id. " 需要手动解决 ");
                    //continue;
                    return false;
                }
            ...
            $cus = \NexaMerchant\Shopify\Models\ShopifyCustomer::where("email", $customer['email'])->first();
            if(is_null($cus)) $cus = new \NexaMerchant\Shopify\Models\ShopifyCustomer();
